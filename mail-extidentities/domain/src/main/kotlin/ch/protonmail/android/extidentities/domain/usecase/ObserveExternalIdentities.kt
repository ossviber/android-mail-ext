...
class ObserveExternalIdentities @Inject constructor(
    private val repository: ExternalIdentityRepository
) {

    operator fun invoke(includeDisabled: Boolean = true): Flow<List<ExternalIdentity>> =
        if (includeDisabled) {
            repository.observeAll()
        } else {
            repository.observeAll().map { identities ->
                identities.filter { it.isEnabled }
            }
        }
}
